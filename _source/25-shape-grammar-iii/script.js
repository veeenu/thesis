...
Mesh.prototype.bind = function(program) {
  ...
  gl.vertexAttribPointer(gl.getAttribLocation(program, 'uv'), 3, gl.FLOAT, false, 0, 0);
...
(function() {
  ...
  (function() {
    var vertices = [], //shgResult.vertices,
        ...
        uvs = []; //shgResult.colors;


    for(var i = 0, I = shgResult.length; i < I; i++) {
      var sr = shgResult[i];
      for(var j = 0, J = sr.vertices.length; j < J; j++) {
        vertices.push(sr.vertices[j]);
        normals.push(sr.normals[j]);
        uvs.push(sr.uvs[j]);
      }
    }

    var bldg = new Mesh(gl, { vertices: new Float32Array(vertices), normals: new Float32Array(normals), uvs: new Float32Array(uvs) });
    bldg.model = model;
    scene.add(bldg);
  }());
